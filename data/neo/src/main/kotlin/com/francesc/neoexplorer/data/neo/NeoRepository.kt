...
interface NeoRepository {
   ...
   * @param endDate Ending date for asteroid search. If omitted, defaults to 7 days after
   *   [startDate].
   ...
   * @return [Result.success] with the asteroid on success, [Result.failure] with the cause on
   *   error.
   */
  suspend fun lookupAsteroid(asteroidId: AsteroidId): Result<NearEarthObject>

  /**
   * NEO-Browse: browse the overall Asteroid data-set as a [PagingData] stream. Uses the Paging 3
   * library; collect the flow and submit to a [androidx.paging.PagingDataAdapter].
   */
